...
import 'package:my_app/pages/ProfilePages/ExportData.dart';
...
class RiwayatDataPage extends StatefulWidget {
  const RiwayatDataPage({
    super.key,
    this.fetchSensorData = RiwayatDataService.fetchSensorHistory,
  });
...
class _RiwayatDataPageState extends State<RiwayatDataPage> {
  ...
  static const String _pondName = 'Kolam A1';
              ...
              return Padding(
                ...
                child: Wrap(
                  ...
                  children: [
                    ...
                    SizedBox(
                      width: contentWidth,
                      child: _buildRingkasanGrid(),
                    ),
                    SizedBox(
                      width: contentWidth,
                      child: _buildGrafikCard(),
                    ),
                    SizedBox(
                      width: contentWidth,
                      child: const Text(
  ...
  Widget _buildGrafikCard() {
    ...
    final rawPoints = _currentRawPoints();
    final rawValues = rawPoints.map((point) => point.value).toList(growable: false);
    final chartPoints = _currentAggregatedPoints();
    final chartValues = chartPoints.map((point) => point.value).toList(growable: false);
    ...
    final latestDataTime = hasData
        ? (rawPoints.isNotEmpty ? rawPoints.last.timestamp : chartPoints.last.timestamp)
        : null;
    final oldestDataTime = hasData
        ? (rawPoints.isNotEmpty ? rawPoints.first.timestamp : chartPoints.first.timestamp)
        : null;
    ...
    return Container(
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          const insightSpacing = 10.0;
          final insightColumns = constraints.maxWidth < 300 ? 1 : 2;
          final insightCardWidth = insightColumns == 1
              ? constraints.maxWidth
              : (constraints.maxWidth - insightSpacing) / 2;
          ...
          return Column(
            ...
            children: [
              ...
              if (_isChartLoading && !hasData)
                ...
                const SizedBox(height: 12),
                Wrap(
                  spacing: insightSpacing,
                  runSpacing: insightSpacing,
                  children: [
                    SizedBox(
                      ...
                      Expanded(
                        child: Column(
                          children: [
                            ...
                            SizedBox(
                              height: 20,
                              child: _buildXAxisLabels(chartPoints),
                            ),
                          ],
                        ),
                      ),
                ...
                Center(
                  child: Text(
                    'Rentang data agregasi: ${_formatDateTimeLabel(oldestDataTime)} - ${_formatDateTimeLabel(latestDataTime)} • $_pondName',
 ...
 Widget _buildTimelineRow(_TimelineItem item, {bool isLast = false}) {
                      ...
                      child: Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              'Export data periode ${_periods[_selectedPeriod].toLowerCase()} untuk $_pondName',
                              style: const TextStyle(
                                color: _textSecondary,
                                fontSize: 14,
                                fontWeight: FontWeight.w500,
                            ...
                            Wrap(
                              ...
                              children: [
                                ChoiceChip(
                                  ...
                                  selected: selectedExportHours == 24,
                                  onSelected: (_) {
                                    setDialogState(() {
                                      selectedExportHours = 24;
                                    });
                                  },
                                ),
                                ChoiceChip(
                                  label: const Text('7 hari'),
                                  selected: selectedExportHours == 168,
                                  onSelected: (_) {
                                    setDialogState(() {
                                      selectedExportHours = 168;
                                    });
                                  },
                                ),
                              ],
                            ...
                            _buildExportOptionButton(
                              ...
                              onPressed: () {
                                ...
                                _exportSensorHistory(
                                  SensorExportFormat.xlsx,
                                  hours: selectedExportHours,
                                );
                              },
                            ),
                            const SizedBox(height: 10),
                            _buildExportOptionButton(
                              ...
                              onPressed: () {
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                    content: Text('Export PDF belum didukung.'),
  ...
  Future<void> _exportSensorHistory(
    SensorExportFormat format, {
    required int hours,
  }) async {
    ...
    try {
      final result = await SensorExportService.exportFromEndpoint(
        format: format,
        hours: hours,
        preferredFolder: SensorExportFolder.downloads,
        shareAfterSave: true,
        openAfterSave: true,
      );
