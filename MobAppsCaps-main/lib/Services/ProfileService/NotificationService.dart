...
class NotificationApiService {
	...
	static Future<List<AppNotificationItem>> getUnreadNotifications({
		...
		final uri = Uri.parse('$resolvedBaseUrl/notifications/unread');
	...
	static Future<int> getUnreadCount({
		...
		final shouldCloseClient = client == null;
		final resolvedBaseUrl = _resolveBaseUrl(overrideBaseUrl);
		final uri = Uri.parse('$resolvedBaseUrl/notifications/unread/count');
		...
		try {
			...
			if (response.statusCode != 200) {
				throw Exception(
					'Failed to load unread notification count (${response.statusCode})',
				);
			}
	...
	static Future<List<AppNotificationItem>> getAllNotifications({
		...
		final shouldCloseClient = client == null;
		final resolvedBaseUrl = _resolveBaseUrl(overrideBaseUrl);
		final uri = Uri.parse('$resolvedBaseUrl/notifications/');
	...
	static Future<Map<String, dynamic>> markAsRead({
		...
		final shouldCloseClient = client == null;
		final resolvedBaseUrl = _resolveBaseUrl(overrideBaseUrl);
		final uri = Uri.parse('$resolvedBaseUrl/notifications/$notificationId/read');
	...
	static Future<Map<String, dynamic>> markAllAsRead({
		...
		final shouldCloseClient = client == null;
		final resolvedBaseUrl = _resolveBaseUrl(overrideBaseUrl);
		final uri = Uri.parse('$resolvedBaseUrl/notifications/read-all');
		...
		try {
			...
			if (response.statusCode < 200 || response.statusCode >= 300) {
					final decoded = response.body.trim().isEmpty ? null : _decodeMap(response.body);
					final message = _extractMessageFrom(decoded) ?? 'Failed to mark all notifications as read';
			...
			final success = (decoded['success'] ?? decoded['ok'] ?? decoded['status']) as dynamic;
...
class AppNotificationItem {
	...
	factory AppNotificationItem.fromMap(Map<String, dynamic> raw) {
		...
		var title = _readString(
			normalized,
			['title', 'subject', 'name', 'type'],
			fallback: 'Notifikasi Baru',
		);
		var message = _readString(
			normalized,
			['message', 'body', 'description', 'detail', 'text'],
			fallback: 'Anda memiliki notifikasi baru dari sistem.',
		);

		// Sanitize title and message to remove unwanted leading mojibake/emoji
		title = _sanitizeLeadingGarbage(title);
		message = _sanitizeLeadingGarbage(message);
	...
	static String _sanitizeLeadingGarbage(String input) {
		var s = input.trim();

		// Remove common replacement characters and control sequences at start
		s = s.replaceFirst(RegExp(r'^[\uFFFD\u0000-\u001F]+'), '');

		// Remove leading non-alphanumeric (including emoji/punctuation) except localized letters
		// Keep basic Latin letters, digits and common accented chars used in Indonesian.
		s = s.replaceFirst(RegExp(r'^[^0-9A-Za-z\u00C0-\u017F]+'), '');
...
String? _extractMessageFrom(Object? decodedBody) {
	if (decodedBody is Map) {
		final map = decodedBody.map((key, value) => MapEntry(key.toString(), value));
