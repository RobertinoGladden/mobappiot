...
class _HomePageState extends State<HomePage> {
  ...
  static const Duration _autoRefreshCooldown = Duration(seconds: 60);
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      _bootstrapDeferredData();
    });
    _scrollController.addListener(_onScroll);
  }

  void _refreshSessionInBackground() {
    if (_hasTriggeredSessionRefresh) {
      return;
    }

    _hasTriggeredSessionRefresh = true;

    Future<void>(() async {
      try {
        await AuthSessionService.checkAndRefreshSession();
      } catch (_) {}
    });
  }
  ...
  Future<void> _bootstrapDeferredData() async {
    if (_deferredBootstrapStarted) {
      return;
    }
    ...
    await _fetchInitialPredictionData();
    _startPredictionPolling();

    await Future<void>.delayed(const Duration(milliseconds: 200));
    if (!mounted) return;

    await _fetchInitialPredictionHistoryData();

    await Future<void>.delayed(const Duration(milliseconds: 200));
    if (!mounted) return;

    await _fetchInitialHarvestEstimateData();
  ...
  Future<void> _fetchInitialFarmCycleData() async {
    try {
      ...
      // Prefer a locally selected cycle stored in SharedPreferences.
      ...
      // If no local selection found, fall back to server's active cycle.
  ...
  void _onScroll() {
    try {
      ...
      // Negative delta means user scrolled up.
      if (delta < -20) {
        final now = DateTime.now().toUtc();
        if (_lastAutoRefreshAt == null || now.difference(_lastAutoRefreshAt!) > _autoRefreshCooldown) {
          _lastAutoRefreshAt = now;
          // Fire-and-forget token refresh (non-blocking UI)
          Future<void>(() async {
            try {
              await AuthSessionService.checkAndRefreshSession();
            } catch (_) {}
  ...
  Widget _buildSensorGrid(BuildContext context, Map<String, dynamic>? sensorData,
      {bool isLoading = false}) {
    final loading = isLoading && sensorData == null;
    ...
    const horizontalPadding = 16.0 * 2; // left + right padding from the list

    // Use SingleChildScrollView + Wrap so card width adapts to screen width
    // while preserving the original 2x2 layout on phones.
    const spacing = crossAxisSpacing;
    final maxWidth = screenWidth - horizontalPadding;
    const minCardWidth = 140.0;
    // Determine how many columns fit based on minCardWidth
    int adaptiveColumns = (maxWidth + spacing) ~/ (minCardWidth + spacing);
    if (adaptiveColumns < 1) adaptiveColumns = 1;
    // Ensure at least 2 columns on typical phones to preserve 2x2 design
    if (screenWidth < 800) {
      adaptiveColumns = 2;
    }

    final cardWidth = (maxWidth - (adaptiveColumns - 1) * spacing) / adaptiveColumns;
        ...
        builder: (context, snapshot) {
          ...
          return LayoutBuilder(
            builder: (context, constraints) {
              ...
              final twoColumn = contentWidth >= 900;
