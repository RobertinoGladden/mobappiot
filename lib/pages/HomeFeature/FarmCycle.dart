...
class _FarmCyclePageState extends State<FarmCyclePage> {
  ...
  Future<void> _submit() async {
    ...
    if (result.success) {
      ...
      if (!isEditing && savedCycle?.id == null) {
        final fallbackCycle = _latestCycleFromList(_cycles);
        if (fallbackCycle?.id != null) {
          await _persistSelectedCycle(fallbackCycle!);
        }
      }
    }
  ...
  FarmCycle? _latestCycleFromList(List<FarmCycle> cycles) {
    if (cycles.isEmpty) {
      return null;
    }
