...
class JadwalPakanCardState extends State<JadwalPakanCard> {
	...
	Widget build(BuildContext context) {
		// prepare schedule widget adaptively so we don't return inside children
		Widget scheduleWidget;
		if (_isLoading) {
			scheduleWidget = const Padding(
				padding: EdgeInsets.symmetric(vertical: 24),
				child: Center(child: CircularProgressIndicator()),
			);
		} else if (_schedules.isEmpty) {
			scheduleWidget = Container(
				width: double.infinity,
			...
			const horizontalPadding = 20.0 * 2; // container padding * 2
			const spacing = 12.0;

			int columns = screenWidth < 600 ? 1 : 2;
			...
			Widget innerList;
			if (columns == 1) {
				// Keep original single-column behavior on phones
				innerList = Column(
					children: _schedules
			...
			// Make the schedule area scrollable within the card and limit its max height
			final maxListHeight = MediaQuery.of(context).size.height * 0.45;
			scheduleWidget = ConstrainedBox(
